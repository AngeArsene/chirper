<form method="POST" action="{{ route("$baseRouteName.like", $message) }}">
    ...
    <button type="submit" class="flex items-center gap-1.5 rounded-full px-3 py-1.5 transition-colors hover:bg-error/10 {{ $textColor }}">
        <svg xmlns="http://www.w3.org/2000/svg" class="size-4" viewBox="0 0 24 24"
            fill="{{ $isLiked ? 'currentColor' : 'none' }}" stroke="currentColor" stroke-width="1.75"
